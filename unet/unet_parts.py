    ...
    def __init__(self, in_channels, out_channels, padding=1, batch_norm=False):
        ...
        if self.batch_norm:
            self.conv_LReLU = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=4, padding=self.padding, stride=2, bias=False),
                nn.BatchNorm2d(out_channels),
                nn.LeakyReLU(inplace=True)
            )
        else:
...
class DiscOut(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DiscOut, self).__init__()
